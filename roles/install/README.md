...
| pgsql_configure_firewalld | <p>Whether to configure firewalld to allow access to the PostgreSQL port.</p><p>If `pgsql_listen_on_local_only` is `true`, this defaults to `false`.</p><p>Otherwise, this defaults to `true` on non-Ubuntu systems.</p> | bool | no |  | true |
| pgsql_configure_ufw | <p>Whether to configure UFW to allow access to the PostgreSQL port.</p><p>If `pgsql_listen_on_local_only` is `true`, this defaults to `false`.</p><p>Otherwise, this defaults to `true` on Ubuntu systems.</p> | bool | no |  | true |
